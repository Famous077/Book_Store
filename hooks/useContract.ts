...
import * as BookReportABI from "@/contract/bookReportABI.json";
...
export const useContract = () => {
  ...
  const addReview = async (bookId: number, review: string) => {
    if (!contract) return;
    const tx = await contract.addReview(bookId, review);
    return await tx.wait();
  };
